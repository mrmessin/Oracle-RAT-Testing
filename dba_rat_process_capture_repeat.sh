###################################################################################
# dba_rat_process_capture_repeat.sh
#
# Parameters:     instance for Replay
#                 Directory location Already Processes Replay
#
# Example: /u01/app/oracle/scripts/dba_rat_process_capture.sh aries1 /zfssa/oracle_export/replay/myreplay
#`
# Description:
...
if (( $# < 2 ));then
  echo "Wrong number of arguments, $*, passed to dba_rat_process_capture_repeat.sh, must pass instance name, directory location of Capture."
  exit 8
fi
...
export LOGFILE=${INST_LOWER}_dba_rat_process_capture_repeat_${DTELOG}.log
...
export replay_name=`echo "${ORACLE_SID}_R2_${DTE}" | awk '{print toupper($0)}'`
export directory_name=${replay_name}
export directory_location=${replay_dir}

echo "Starting the Real Application Testing Replay Processing......."
echo "--"
...
echo "Processing Captured Workload in replay location ${directory_location}"
echo "Processing Captured Workload in replay location ${directory_location}" >> ${LOG}
######################################
# Process Workload in Replay Location
######################################
...
--exec dbms_workload_replay.prepare_replay(synchronization=>'TIME',capture_sts=>FALSE, query_only=TRUE);
...
echo "Process of prepared Capture for Replay Completed"
echo "Process of prepared Capture for Replay Completed" >> ${LOG}
echo "Used Replay Directory Name ${directory_name}" >> ${LOG}
echo "Used Replay Directory Name ${directory_name}"
echo "Used Replay Directory Location ${directory_location}" >> ${LOG}
echo "Used Replay Directory Location ${directory_location}"
